    ...
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
        ...
        <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
            <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
                <div>
                    <h5 class="mr-3 font-semibold dark:text-white">Service List</h5>
                    <p class="text-gray-500 dark:text-gray-400">Manage all your existing services or add a new one</p>
                </div>
                ...
                <div
                    ...
                    <button type="button" id="createServiceModalButton" data-modal-target="createServiceModal"
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    @foreach($services as $service)
                        <tr class="border-b dark:border-gray-700" id="service-list-service-id-{{$service->id}}">
                            <th scope="row"
                            ...
                            <td class="px-4 py-3">
                                @switch($service->type)
                                    @case($service->type==='RC')
                                        Reccuring charge
                                        @break
                                    @case($service->type==='NRC')
                                        One time charge
                                        @break
                                @endswitch
                            </td>
                            ...
                            <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white float-right">
                                <div class="flex items-center space-x-4">
                                    <a id="editServiceModalButton" data-modal-target="editServiceModal-{{$service->id}}"
                                    ...
                                    <a id="PreviewServiceModalButton"
                                       data-modal-target="showServiceModal-{{$service->id}}"
                                       data-modal-toggle="showServiceModal-{{$service->id}}"
                                    ...
                                    <a id="DeleteServiceModalButton"
                                       data-modal-target="DeleteServiceModal-{{$service->id}}"
                                       data-modal-toggle="DeleteServiceModal-{{$service->id}}"
...
<script>
    const servicesList = @json($services->items());
    document.getElementById('service-search').oninput = function(e) {
        const searchValue = e.target.value;
        const filteredServices = servicesList.filter(service => {
            return service.name.toLowerCase().includes(searchValue.toLowerCase());
        });

        const servicesToHide = servicesList.filter(service => {
            return !filteredServices.includes(service);
        });
        const servicesToShow = servicesList.filter(service => {
            return filteredServices.includes(service);
        });

        servicesToHide.forEach(service => {
            document.getElementById(`service-list-service-id-${service.id}`).style.display = 'none';
        });
        servicesToShow.forEach(service => {
            document.getElementById(`service-list-service-id-${service.id}`).style.display = 'table-row';
        });
    }
</script>
